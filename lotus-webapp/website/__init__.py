...
db = SQLAlchemy()
db_name = 'db.sqlite3'
# admin = Admin(name='Lotus', template_mode='bootstrap3') #-> Commented as i built custom admin pages
...
def create_app():
    ...
    app = Flask(__name__)
    # set secret key for encrypting sessions
    app.config['SECRET_KEY'] = '@Th3NorthR3m3mbers'
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{db_name}'
    # app.config['FLASK_ADMIN_SWATCH'] = 'flatly'	#set theme for admin panel -> Commented as i built custom admin pages

    db.init_app(app)  # initialize database
    # admin.init_app(app)		#initialize admin -> Commented as i built custom admin pages

    # initialize the login manager for session and token mngmt
    login_manager = LoginManager()
    login_manager.init_app(app)
    # what does this do? Tell app where login view is. If user is not logged in redirect here. Else returns an error
    ...
    # had to remove the prefix to allow for displaying of images
    app.register_blueprint(admin, url_prefix='/')

    # build and admin panel views
    # -> Commented as i built custom admin pages
